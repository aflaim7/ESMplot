...
# Difference labels for the 4 forcing experiments (indices 1-4 minus index 0)
...
for season_name, MON in SEASONS.items():
    ...
    # Result: [n_forcing x lat x lon] where n_forcing = 4
    #-----------------------------------------------------------------
    ...
    if PLOT_PRECT:
        ...
        if overlay_vec:
            u_p = (u_diff.values)
            v_p = (v_diff.values)
            p95_i = np.nanpercentile(np.sqrt(u_p**2 + v_p**2), 95)
        ...
        plot_contour_map_avg(
            ...
            u=xr.DataArray(u_p, dims=prect_diff.dims, coords=prect_diff.coords) if overlay_vec else None,
            v=xr.DataArray(v_p, dims=prect_diff.dims, coords=prect_diff.coords) if overlay_vec else None,
    ...
    if PLOT_d18Op:
        ...
        if overlay_vec:
            vec_scale_o = vec_scale_p  # same winds, reuse scale computed above
        else:
            u_p, v_p = None, None
        ...
        plot_contour_map_avg(
            ...
            LatMin=LatMin, LatMax=LatMax, LonMin=LonMin, LonMax=LonMax,
            overlay_vec=overlay_vec,
            u=xr.DataArray(u_p, dims=d18Op_diff.dims, coords=d18Op_diff.coords) if overlay_vec else None,
            v=xr.DataArray(v_p, dims=d18Op_diff.dims, coords=d18Op_diff.coords) if overlay_vec else None,
