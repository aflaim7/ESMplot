...
from ESMplot.watertagging.tagged_regions_cenlon_LGMregions import draw_land_tags, draw_ocean_tags
...
def watertagging_values_on_map(
  ...
  def _to_proj_lon(lon_east_deg: float, central: float = 0.0) -> float:
      """Longitude expressed in PlateCarree(central_longitude=central) domain [-180, 180]."""
      return (lon_east_deg - central + 180.0) % 360.0 - 180.0

  def _draw_region_box_wrapped(ax, slat, nlat, wlon_geo, elon_geo, linestyle, edgecolor, linewidth, zorder, central):
      """
      Draw lon/lat rectangle on an axis whose projection may be Pacific-centered.
      Inputs wlon_geo/elon_geo are geographic degrees east in [-180, 180] (we normalize just in case).
      If the rectangle crosses the projection wrap, draw two pieces.
      Assumes draw_region_box uses geographic coordinates with transform PlateCarree(central=0).
      """
      W = _to_geo_lon(wlon_geo)
      E = _to_geo_lon(elon_geo)
      Wp = _to_proj_lon(W, central)
      Ep = _to_proj_lon(E, central)

      def _draw_geo(w_, e_):
          draw_region_box(ax=ax, slat=slat, nlat=nlat, wlon=w_, elon=e_,
                          linestyle=regline, facecolor='none',
  ...
  def _add_latlon_labels(ax, show_bottom=False, show_left=False, xlocs=None, ylocs=None):
      ...
      if xlocs is None:
          xlocs = np.arange(-180.0, 180.0 + 1e-9, gl_xstep)
      if ylocs is None:
  ...
  # =========================
  # Projection handling (updated)
  # =========================
  # If user asked for Pacific-centered and projection is PlateCarree, rebuild with central_longitude=180
  if central_lon_180 and isinstance(proj, ccrs.PlateCarree):
      proj = ccrs.PlateCarree(central_longitude=180.0)

  # Save the projection's central longitude for later logic
  ...
  # ----------------------
  # Extent in the axis' CRS domain (use [-180,180] for the axis' projection)
  for ax in (ax1, ax2, ax3, ax4, ax5, ax6):
      ax.set_extent([-180.0, 180.0, LatMin, LatMax],
                    crs=ccrs.PlateCarree(central_longitude=_proj_central))
  ...
  # Plot land and ocean base colors (data are geographic; transform is standard PlateCarree)
  ...
  # ----------------------
  # Gridline labels (updated ticks)
  # ----------------------
  if show_latlon:
      eps = 1e-7
      x_all = np.arange(-180.0, 180.0 + eps, gl_xstep)
      ...
      x_left  = x_all[x_all <  (180.0 - eps)]
      x_mid   = x_all[(x_all > (-180.0 + eps)) & (x_all < (180.0 - eps))]
      x_right = x_all[x_all >  (-180.0 + eps)]
  ...
  # Define perimeter (normalize to geographic [-180,180] for drawing)
  rlats = np.float64(lat[lats]-latadj)
  rlatn = np.float64(lat[latn]+latadj)
  rlonw = _to_geo_lon(np.float64(lon[lonw]-lonadj))
  rlone = _to_geo_lon(np.float64(lon[lone]+lonadj))

  # Draw rectangles on all panels (auto-splits if crossing the wrap)
  for a in (ax1, ax2, ax3, ax4, ax5, ax6):
      _draw_region_box_wrapped(a, slat=rlats, nlat=rlatn,
                               wlon_geo=rlonw, elon_geo=rlone,
                               linestyle=regline, edgecolor=regcol,
                               linewidth=reglw, zorder=10, central=_proj_central)
  ...
  # Note: we keep text coordinates in geographic lon/lat with transform=PlateCarree(),
  # letting Cartopy project them into the axis CRS (works for central_lon_180 too).
  ...
  for o in range(num_landtags,len(tagnames),1):
      idx = o - num_landtags
      ax2.text(oceanlon[idx],oceanlat[idx],text_precip[o].round(rnd_precip),fontsize=tag_fs,
               transform=ccrs.PlateCarree(), zorder=text_zorder,
               ha='center',va='center',bbox=dict(boxstyle='square',pad=bckgrnd_pad,edgecolor='none',facecolor=bckgrnd_col))
      ax4.text(oceanlon[idx],oceanlat[idx],text_pctpr[o].round(rnd_pctpr),fontsize=tag_fs,
               transform=ccrs.PlateCarree(), zorder=text_zorder,
               ha='center',va='center',bbox=dict(boxstyle='square',pad=bckgrnd_pad,edgecolor='none',facecolor=bckgrnd_col))
      if text_d18Op[o] > 0:
          ax6.text(oceanlon[idx],oceanlat[idx],'+'+str(text_d18Op[o].round(rnd_d18Op)),color='r',
                   transform=ccrs.PlateCarree(), zorder=text_zorder,
                   fontsize=tag_fs,ha='center',va='center',bbox=dict(boxstyle='square',pad=bckgrnd_pad,edgecolor='none',
                                                                     facecolor=bckgrnd_col))
      elif text_d18Op[o] < 0:
          ax6.text(oceanlon[idx],oceanlat[idx],text_d18Op[o].round(rnd_d18Op),color='b',fontsize=tag_fs,
                   transform=ccrs.PlateCarree(), zorder=text_zorder,
                   ha='center',va='center',bbox=dict(boxstyle='square',pad=bckgrnd_pad,edgecolor='none',facecolor=bckgrnd_col))
      else:
          ax6.text(oceanlon[idx],oceanlat[idx],text_d18Op[o].round(0),color='k',fontsize=tag_fs,
                   transform=ccrs.PlateCarree(), zorder=text_zorder,
                   ha='center',va='center',bbox=dict(boxstyle='square',pad=bckgrnd_pad,edgecolor='none',facecolor=bckgrnd_col))
